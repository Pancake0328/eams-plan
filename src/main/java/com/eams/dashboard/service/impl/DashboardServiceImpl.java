...
public class DashboardServiceImpl implements DashboardService {
        ...
        public List<AssetDistribution> getDepartmentDistribution() {
                ...
                // 按部门ID分组统计
                Map<Long, List<AssetInfo>> departmentGroup = allAssets.stream()
                                .filter(asset -> asset.getDepartmentId() != null)
                                .collect(Collectors.groupingBy(AssetInfo::getDepartmentId));

                List<AssetDistribution> distributions = new ArrayList<>();
                long totalAssets = allAssets.size();

                // 查询所有部门信息
                Map<Long, String> departmentMap = departmentMapper.selectList(null).stream()
                                .collect(Collectors.toMap(
                                                com.eams.system.entity.Department::getId,
                                                com.eams.system.entity.Department::getDeptName));

                for (Map.Entry<Long, List<AssetInfo>> entry : departmentGroup.entrySet()) {
                        Long departmentId = entry.getKey();
                        String departmentName = departmentMap.getOrDefault(departmentId, "未知部门");
                        ...
                        distributions.add(new AssetDistribution(departmentName, count, amount, percentage));
                }
