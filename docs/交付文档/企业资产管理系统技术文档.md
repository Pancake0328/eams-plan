...
客户端                                    服务端
  ...
  │                              3. 生成 JWT Token（有效期由 jwt.expiration 配置）
  │                              4. 将用户权限信息加载到应用内存缓存（key: userId）
  │                                          │
  ...
  │                              2. 解析 JWT，验证签名和过期时间
  │                              3. 从 JWT Claims 中提取 username
  │                              4. 从应用内存缓存或 DB 加载用户权限
...
public class JwtUtils {
    ...
    public String generateToken(String username) {
        Date expireDate = new Date(System.currentTimeMillis() + expirationSeconds * 1000);
        return Jwts.builder()
                .subject(username)
                .issuedAt(new Date())
...
#### 9.1.1 缓存使用场景（Redis + 应用内存）

| 缓存层 | 缓存 Key | 缓存内容 | TTL | 失效策略 |
|----------|----------|----------|-----|----------|
| 应用内存 | `local:user:permissions:{userId}` | 用户权限列表 | 当前进程生命周期 | 用户角色/角色权限/菜单权限变更时主动删除 |
| Redis | `menu:tree` | 全量菜单树 | 1小时 | 菜单变更时主动删除 |
| Redis | `dept:list` | 部门列表 | 1小时 | 部门变更时主动删除 |
| Redis | `category:tree` | 资产分类树 | 2小时 | 分类变更时主动删除 |
| Redis | `dashboard:overview` | 统计概览数据 | 5分钟 | 定时过期 |

#### 9.1.2 缓存更新模式

采用“应用内存权限缓存 + Redis业务缓存”的分层缓存模式：

```
权限读取流程：
  1. 先查应用内存权限缓存 → 命中直接返回
  2. 未命中 → 查 MySQL → 回填应用内存缓存 → 返回

权限变更流程：
  1. 执行角色分配/角色权限分配/菜单权限变更
  2. 计算受影响用户并主动删除其权限缓存
  3. 下次请求自动重建权限缓存（热加载）

业务数据缓存流程（Redis）：
  1. 读取先查 Redis，未命中回源 MySQL
  2. 写入先更新 MySQL，再删除对应 Redis Key
```
...
- 针对采购创建、采购入库、取消采购、盘点明细创建等高频批量场景，新增统一分片执行器，默认批次大小为 500；
- 资产信息、生命周期记录、角色菜单、用户角色等表新增批量写入/批量更新 SQL，减少逐条写库造成的连接与网络往返；
- 资产编号生成支持按前缀一次分配编号区间，避免逐条加锁生成导致的吞吐下降；
- 在大批量入库（数百~数千条）场景下，可显著降低接口阻塞时间并提升并发下稳定性。
...
| v1.1.0 | 2026-03 | 新增批量写库优化、权限内存缓存与热加载、端口环境变量覆盖（SERVER_PORT）等迭代说明 |
